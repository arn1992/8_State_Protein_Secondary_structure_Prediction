...
t1 = time.time()
train_data = np.load('data/cullpdb+profile_6133_filtered.npy.gz')
t2 = time.time()

print
type(train_data), len(train_data), train_data.shape

print
'Memory:', sys.getsizeof(train_data) / (1024 * 1024), 'M'
print
'load 6133_filtered Runtime:', '%.2f' % (t2 - t1), 'seconds'

train_data = np.reshape(train_data, (-1, 700, 57))
print
'train_data', type(train_data), len(train_data), train_data.shape
...
t0 = time.time()
test_data = np.load('data/cb513+profile_split1.npy.gz')
t00 = time.time()
# print  'test_data',type(test_data), len(test_data), test_data.shape
print
'load cb513 Runtime:', '%.2f' % (t00 - t0), 'seconds'
test_data = np.reshape(test_data, (-1, 700, 57))
print
'test_data', type(test_data), len(test_data), test_data.shape
...
print
'test_label', type(test_label), len(test_label), test_label.shape
...
from keras.layers import Input, Embedding, LSTM, Dense, merge, Convolution2D, GRU, TimeDistributed, Reshape, \
...
def build_model():
    ...
    aux_output = TimeDistributed(Dense(8, activation='softmax', name='aux_output')(protein_features))
...
#####Training the model
history = model.fit({'aux_input': trainpssm}, {'aux_output': trainlabel},
                    validation_data=({'aux_input': valpssm}, {'aux_output': vallabel}),
                    nb_epoch=1, batch_size=42, callbacks=[checkpointer, earlyStopping], verbose=2, shuffle=True)
plotLoss(history)
