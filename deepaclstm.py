...
from keras.layers import Input, Embedding, LSTM, Dense, merge, Convolution2D, GRU, TimeDistributed, Reshape, MaxPooling2D, Convolution1D, BatchNormalization, concatenate
...
from data_process import load_cul6133_filted, load_cb513
...
def build_model():
    ...
    print
    auxiliary_input.get_shape()
    concat = concatenate([x, auxiliary_input], axis=-1)
    ...
    lstm_f1 = LSTM(300, return_sequences=True, inner_activation='sigmoid', dropout=.5)(conv2_features)
    lstm_b1 = LSTM(300, return_sequences=True, go_backwards=True, inner_activation='sigmoid', dropout=.5)(conv2_features)

    lstm_f2 = LSTM(300, return_sequences=True, inner_activation='sigmoid', dropout=.5)(lstm_f1)
    lstm_b2 = LSTM(300, return_sequences=True, go_backwards=True, inner_activation='sigmoid', dropout=.5)(lstm_b1)

    concat_features = concatenate([lstm_f2, lstm_b2, conv2_features],  axis=-1)
...
history = model.fit({'main_input': traindatahot, 'aux_input': trainpssm}, {'main_output': trainlabel},
                    validation_data=({'main_input': valdatahot, 'aux_input': valpssm}, {'main_output': vallabel}),
                    nb_epoch=200, batch_size=32, callbacks=[checkpointer, earlyStopping], verbose=2, shuffle=True)
# plotLoss(history)
model.load_weights(load_file)
print
"#########evaluate:##############"
score = model.evaluate({'main_input': testdatahot, 'aux_input': testpssm}, {'main_output': test_label}, verbose=2,
                       batch_size=2)
print (score)
