...
'''
save_best = ModelCheckpoint(filepath='D:/algorithms for bioinformatics/final project rnn/cnn/checkpoint-{val_acc:.4f}.h5',monitor='val_acc', mode='auto', save_best_only=True)
learning_rate_reduction = ReduceLROnPlateau(monitor='val_loss', patience=7, verbose=1, factor=np.sqrt(0.1),min_lr=0.5e-6, cooldown=0, mode='auto')

callbacks=[learning_rate_reduction,save_best,call_b]
'''
